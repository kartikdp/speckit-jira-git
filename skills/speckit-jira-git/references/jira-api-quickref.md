...
story and `PROJ-101..PROJ-N` for sub-task keys. Review activity uses the
actual human reviewer's identity. PR Review tasks remain unassigned unless a
human reviewer is explicit. Never use an automation or service identity as a
human reviewer or assignee.
...
POST /rest/api/3/issue
{
  "fields": {
    ...
    "summary":     "[05.01] Code Review Round 1",
    "parent":      { "key": "PROJ-100" },
    "assignee":    { "accountId": "..." },
...
Attach a Spec Kit artifact to an issue:
...
Use `JiraClient.attach_file()` rather than hand-building this request. Before uploading, fetch the issue's `attachment` field and skip an existing attachment with the same filename and byte size. Specs Review requires the core `spec.md`, `plan.md`, and `tasks.md` plus available supporting research, data model, quickstart, checklist, and referenced contracts.
