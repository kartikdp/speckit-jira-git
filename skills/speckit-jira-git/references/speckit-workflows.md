...
Generated sub-tasks remain in Jira's default creation status. Do not transition them from `[X]` source checkboxes, historical completion, merged PRs, parent status, or current PR state. A transition requires an explicit user request naming the target status.
