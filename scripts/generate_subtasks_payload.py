...
def main() -> None:
    ...
    ap.add_argument(
        "--summary-prefix",
        default=None,
        help="Prefix for each sub-task summary. Defaults to `[<parent-key>][Phase N]`",
    )
    ap.add_argument(
        "--extra-label",
        action="append",
    ...
    for p in phases:
        phase_num = p["num"]
        title = p["title"]
        prefix = args.summary_prefix or f"[{args.parent}]"
        summary = f"{prefix}[Phase {phase_num}] {title}"
        if len(summary) > 240:
            summary = summary[:237] + "..."

        labels = ["speckit-phase", f"phase-{phase_num}"] + args.extra_label
